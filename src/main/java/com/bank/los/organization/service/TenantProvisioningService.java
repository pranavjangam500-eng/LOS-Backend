...
import com.bank.los.config.TenantDataSourceProvider;
...
public class TenantProvisioningService {
    ...
    public void provisionTenant(String tenantDb, String orgCode, String orgName, String host, Integer port) {
        log.info("Provisioning tenant database and default entities for orgCode={}, db={}", orgCode, tenantDb);
        try {
            // 1. Create/Connect DataSource and execute tenant-schema.sql
            tenantDataSourceProvider.getOrCreateTenantDataSource(tenantDb, host, port);

            // 2. Set context to the new tenant DB
            TenantContext.setCurrentTenant(tenantDb);
            TenantContext.setCurrentOrgCode(orgCode);

            // 3. Seed default roles
            TenantRole adminRole = seedRole("ADMIN", "BANK_NBFC", "Bank/NBFC internal admin — configures org and manages users");
            ...
            seedPermissions();
            seedRolePermissions(tenantDb, adminRole, List.of(
            ...
            seedRolePermissions(tenantDb, makerRole, List.of(
            ...
            seedRolePermissions(tenantDb, checkerRole, List.of(
                    ApplicationConstants.Permissions.LOAN_APPLICATION_VIEW,
            ...
            seedRolePermissions(tenantDb, viewerRole, List.of(
                    ApplicationConstants.Permissions.LOAN_APPLICATION_VIEW,
                    ApplicationConstants.Permissions.CUSTOMER_VIEW,
                    ApplicationConstants.Permissions.CUSTOMER_VIEW_ALL,
                    ApplicationConstants.Permissions.DOCUMENT_VIEW,
            ...
            log.info("Tenant DB provisioning completed successfully for orgCode={}", orgCode);
        } catch (Exception ex) {
            log.warn("Notice during tenant provisioning for {}: {}", orgCode, ex.getMessage());
        } finally {
            TenantContext.clear();
        }
    }
    ...
    private void seedRolePermissions(String tenantDb, TenantRole role, List<String> permCodes) {
        ...
        DataSource ds = tenantDataSourceProvider.getTenantDataSource(tenantDb);
        if (ds == null) return;
